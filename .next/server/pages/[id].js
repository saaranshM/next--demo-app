...
const getStaticPaths = async () => {
  const client = await external_mongodb_["MongoClient"].connect("mongodb+srv://admin:test1234@cluster0.r0zao.mongodb.net/meetups?retryWrites=true&w=majority");
  const db = client.db();
...
const getStaticProps = async context => {
  ...
  const meetup = await meetupsCollections.findOne({
    _id: Object(external_mongodb_["ObjectId"])(id)
  });
  client.close();
  return {
    props: {
...
/* harmony default export */ var _id_ = __webpack_exports__["default"] = (MeetupDetails);
